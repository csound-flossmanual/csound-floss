...
As a short survey of some *csoundmagics* and *iCsound* we start again with loading the library and creating an instance:
...
In the first section, we see the usual way to load the modules, create a csound instance and send an instrument to it. The *layout* section defines the widgets which will be present in the GUI. The `key` parameter is particularily important here, as this is the way a widget can be identified. 

The most important section is the `while` loop, or event loop. Here we send the values of the browse button and the slider to csound:
...
The second part of this chapter is based on Andrés Cabrera's article *Using Python inside Csound, An introduction to the Python opcodes*.[^1] All examples below are to be executed in a Terminal. If using CsoundQt, choose *Run in Term* instead of *Run*.
