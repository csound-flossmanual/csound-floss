...
You learned in [Tutorial 01](/get-started/GS-01#the-csound-document-structure)
 that the Csound file consists of three sections:
...
code the instruments, in the `CsScore` we call them, and the `CsOptions` 
...
The instrument definitions, or orchestra, were collected in an *.orc*  file, 
and the score lines were collected in a *.sco* file. The options were put in
...
fourth position then follow the *.orc* and *.sco* file.
...
It is not only a different role. It can be said that an instrument knows some
about the score. As you learned in [Tutorial 04](/get-started/GS-04)
and [Tutorial 07](/get-started/GS-07), each instrument
instance knows about the *p-fields* which created this instance. It knows about
...
We will introduce at first `schedule`, and then in [Tutorial 09](/get-started/GS-09) 
come to `schedulek`. 
...
As `schedule` is an opcode, its input arguments are **separatd by commas**.
...
because `schedule` only instantiates *instrument* events whilst a score line
...
`iWhatIsYouNumber = nstrnum("Hello")`
...
above, and run it again. Not it should stop after 20 seconds.


## The Csound Reference Manual as companion and field for improvements

All information about the usage of the opcodes is collected in the
...
with the next tutorial: [08 Hello If](01-GS-09.md).
...
use the `event_i` opcode, and its *k-rate* version `event`. 
...
the *Play* instrument which plays a sine tone for three seconds. Then we call the 
*Print* instrument which displays its message in the console and calls the
*Terminate* instrument after three seconds. This instrument then terminates
