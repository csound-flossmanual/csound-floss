...
You learned in [Tutorial 02](/get-started/GS-02#the-csound-document-structure)
 that the Csound *.csd* file consists of three sections:
...
initialization pass. In other words: All *i-rate* statements will work, but 
no *k-rate* or *a-rate* one.
...
a *legato* line; so no reason for more than one instance at the same time.
