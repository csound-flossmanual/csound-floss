...
We will introduce at first `schedule`, and then
in [Tutorial 11](/get-started/GS-11) come to `schedulek`.
...
Note 2: It is up to you whether you add spaces to the commas in
the `schedule` argument list, or not. For Csound, the commas are the separator.
Once you accept this, you can use spaces or tabs in addition, or not.
So these two lines are both valid:
...
The following example simply transfers the score lines
from [Tutorial 07](/get-started/GS-07) to `schedule` statements.
...
- Internally, Csound converts all instrument names to positive integer numbers.
  You can get this number via the `nstrnum` opcode.
  Put the code `iWhatIsYourNumber = nstrnum("Hello")` anywhere
  in the `CsInstruments` section, for instance below the `schedule`lines.
...
When we use a traditional score, as we did
in [Tutorial 07](/get-started/GS-07#example), Csound will
...
So usually we want this "endless" performance. (According to
the [Csound Manual](https://csound.com/docs/manual/ScoreTop.html) this is about
...
All information about the usage of an opcode is collected in
the [Csound Reference Manual](https://csound.com/docs/manual/index.html).

Every opcode has an own page here. Please have a look at the page for
the `schedule`opcode which is at&nbsp;
...
if we can, for instance in opening a ticket
at [Github](https://github.com/csound/manual/issues) or by suggesting an improvement
...
which we somehow replaced in this tutorial by the `schedule` opcode in
...
And in this book, the Csound FLOSS Manual, we have a chapter
about [methods of writing scores](14-a-methods-of-writing-csound-scores.md), too.
...
not possible when we fire a score event from inside the orchestra.
So the `t` statements and similar will not work.
...
the _Play_ instrument which plays a sine tone for three seconds. Then we call
the _Print_ instrument which displays its message in the console and calls
the _Terminate_ instrument after three seconds. This instrument then terminates
...
as special symbol for the duration. According to
the [Csound Reference Manual](https://csound.com/docs/manual/ScoreTop.html),
...
instrument whichs turns off all instances of instrument "Zett". We will
use `turnoff2` or `turnoff2_i` more later.
