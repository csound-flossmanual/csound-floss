...
Using `if` is probably the most important syntactical unit in any programming
language. The good thing is that it is so common for us all in daily life,
so all be must do is to transfer it to a programming language, in this case
to Csound.
