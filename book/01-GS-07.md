...
LATER " Three instrument instances in a sequence ../resources/images/01-GS-07-a.png"
...
LATER "Three instrument instances overlapping each other  ../resources/images/01-GS-07-b.png"
...
LATER " Instrument reading p4 and p5 of a sore line  ../resources/images/01-GS-07-c.png"



## Direct insertion or variable definition

We have used **p4** and **p5** in the code above to define the *i-rate* 
...
Already in Max Mathew's MUSIC IV in 1963 there was a *score* with a list of 
parameters. And there were already *instruments* which could refer to a 
certain "parameter field" by a number like `P4`.

IMAGE?

Similar in MUSIC V which Jean-Claude Risset used to write his epochal 
"Catalogue of Computer Synthesized Sounds" in 1969. His code can with only 
slight modifications be transformed into Csound code. 

IMAGE?

The "C" in Csound points to the 
...
*P fields* are on one hand simple and give a lot of possibilities. There are, on the
other hand, restrictions. Basically, a *p-field* carries a number. It took a 
lot of work from the Csound developers to make it possible to write a string in a 
p-field, too. But it is still not possible to pass a signal via a *p-field*
to an instrument.

Fortunately, *p-fields* are only one possibility for an instrument to communicate
