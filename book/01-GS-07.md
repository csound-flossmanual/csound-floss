...
Already in Max Mathew's MUSIC IV in 1963 there was a _score_ with a list of
parameters. And there were already _instruments_ which could refer to a
...
- Add two p-fields in the score to specify the first and the last volume as _dB_.
  ...
  Introduce two new variables and call them _iDbStart_ and _iDbMid_.
...
- Go back to the first code by reloading the page. Now remove the fifth p-field
  from the score and change the code in the instrument so that the _iMidiEnd_
  variable is always 6 MIDI notes lower than _iMidiStart_.
...
is the first _p-field_ `"LiveInput"` a string, and not a number. And we cannot
