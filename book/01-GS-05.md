...
All you must know about MIDI keys or note numbers: _C4_ is set to note number 60. And then each semitone, or each next key on a MIDI keyboard,
...
use this opcode for printing _k-rate_ or _a-rate_ variables.  
...
6. Code a "chromatic scale" (= using always the next MIDI note) which falls from
   _D5_ to _A4_. Each MIDI key stays for one second, and then immediately moves
...
We see that _Freqs_1_ reaches 440 Hz after half of the duration, whilst _Freqs_2_
...
needs 6 seconds. The
_Pitches_2_ line, however, reaches MIDI note 69 after 8 seconds, therefore
