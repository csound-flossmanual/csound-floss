...
For a programming language this means to *print*. Via printing the program shows 
...
MIDI note 68 (Ab4). This MIDI note line we store in the Variable *kMidi*:
...
I'd recommend to only use [ASCII characters](https://en.wikipedia.org/wiki/ASCII) 
in Csound to avoid problems.
...
The *kFreqLine_1* variable contains the frequency signal which derives from a linear
transition in the MIDI note domain. The *kFreqLine_2* variable contains the
...
both signals. For *kFreqLine_1*, this is the *kMidiLine_1* signal which we created.
...
get MIDI note numbers as output. So to get the corresponding MIDI pitches to
*kFreqLine_2*, we write:
...
This was not always like this in european music tradition, and as far as I know
also in music traditions in other cultures. There was mostly a certain range in 
which the standard pitch could vary. Even in 19th century, when standardization
step by step took place, this was still the case. The standard pitch was at first
internationally fixed on a conference in Vienna 1885. 
...
from one semitone to the next the frequency ratio is ${\sqrt[12]{2}}$ or
$2^{1/12}$.
...
And indeed this complies with the result of `mtof`:
