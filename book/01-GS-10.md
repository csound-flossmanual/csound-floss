...
Throwing the dies and inventing games which follow their result has always
...
For getting a random number, we set one limit for the largest possible number,
...
is there, all other numbers are determined by the 
...
the space for the actual Csound code in a *.csd* document is what is enclosed
...
Sometimes this global space is called "instrument 0". The reason is that the
instruments in the orchestra have 1 as smallest possible number. 

What we can do in this global space:

- We can set global parameters as sample rate etc, and also the `seed` because
this is a global parameter, too.
- We can define own functions or import external code.
- We can create function tables (buffers) and assign software channels.
- We can perform *i-rate* expressions. Insert, for instance, 
`prints("Hello Global Space!\n")` in the global space and look in the console
ouput.

What we cannot do in the global space:

- We cannot perform any *k-rate* or *a-rate* expressions.
...
instr RandomWalk
  ...
  if (iCount > 1) then
    ...
    iStart = p3
    //next duration is plusminus half of the current
    iDur = p3 + random:i(-p3/2,p3/2)
    //next pitch is plusminus a semitone
    iNextPitch = iMidiPitch + random:i(-1,1)
    //next volume is plusminus 2dB but
...
Random is for our phantasy. By introducing any "if", we can change conditions in
...
- `random:i(iMin,iMax)`
- `seed(iNum)` 
- *global space* is the part of the `CsInstruments` section which is outside
...
Perhaps you noticed that until now we only used random at *i-rate*.
...
We even can use the `random` opcode at *a-rate*. Then we generate one random
...
instrument event (*i-rate*) and about one thousand different random values
per second (*k-rate*) is not enough for many use cases.
Imagine we want a sound to move between two speakers. We may want it to change
its direction about once per second.

This is a typical case which is covered by the `randomi` opcode.
...
FIGURE about randomi:k(0,1,1,3)

Sometimes we want the random values to be held until the next one. This is
the job of then `randomh` opcode. Here comes a comparision between 
`randomi` and `randomh`:

FIGURS same randomi as --- but also with randomh as normal lines
...
is the mathematical formulation for it, and it is implemented in the `gauss`
opcode in Csound.

And the *random walk* is only one possibility of a random which depends on
previous step(s). A Markov chain is another example for random which is more
interaction-based than throwing the dies or generating noise.
