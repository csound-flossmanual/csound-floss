...
This is a simple example of a *random walk*.

This means that the random values of the next step depend on the previous step.

In the example above, note number seven with a duration of 11.8 seconds 
would not have been possible in one of the previous steps.

In nature, usually a random walk is not joined by a tendency.
Instead, Interesting patterns derive only from simple settings but a big
number of random steps. Have a look at the 
[Wikipedia article](https://en.wikipedia.org/wiki/Random_walk) or other sources.

But in art we do not copy nature. We communicate with nature in elaborating
our phantasies.
