...
thing in the world reliable, each *must read* consists of five headings, and each
...
go through one Tutorial, is one hour. So once I can finish 24 Tutorials finally 
the goal can be fulfilled to **learn Csound in one day**. 

Nevertheless, I must admit that this *Getting Started* can only be one amongst many. 
Its focus is on
...
exquisite and subtle sounds, it be fixed media or the fastest possible real-time
application, it be standalone or as a plugin to your preferred DAW.
As for sounds, please have a look, and in particular an ear to
Iain McCurdy's examples, either on his 
[website](http://iainmccurdy.org/csound.html)
or inside [Cabbage](https://cabbageaudio.com/).
They are an inexhaustible source of inspiration and a *must have* level of
sound quality.
